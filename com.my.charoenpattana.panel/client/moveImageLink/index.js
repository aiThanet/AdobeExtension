...
$("#confirm").on("click", e => {
  if (confirm("ต้องการเริ่มดำเนินการหรือไม่")) {
    ...
    jsx.evalScript(`startMoveAllLink(${JSON.stringify(files)})`, res => {
      ...
      data = JSON.parse(res);
      $("#displayBody").empty();
      $("#displayBody")[0].innerHTML = "<div class='row mx-1 my-2'><h2 class='text-center'>ไฟล์ที่ไม่ได้ใช้งาน</h2><div><table class='table'><thead><tr><th>ไฟล์</th></tr></thead><tbody id='resultDisplay'></tbody></table></div>";
